...
string Vehicle::_get_next_state(map<int,vector < vector<int> > > predictions){
  ...
  for(int i = 0; i < states.size(); i++){
      ...
      trajectory = _trajectory_for_state(states[i], predictions_copy);
      int cost = calculate_cost(trajectory, predictions);
      costs[states[i]] = cost;
  }
...
vector<Snapshot> Vehicle::_trajectory_for_state(string state, map<int,vector < vector<int> > > predictions, int horizon = 5){
...
void Vehicle::_restore_state_from_snapshot(Snapshot s){
    this->lane = s.lane;
    this->s = s.s;
    this->v = s.v;
    this->a = s.a;
